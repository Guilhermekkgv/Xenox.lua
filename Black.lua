...
local Linux = {
    Theme = {
        ...
        TextBox = Color3.fromRGB(28, 28, 28),
...
function Linux:Notify(config)
    ...
    local Notification = Linux.Instance("Frame", {
        ...
        BorderColor3 = Linux.Theme.Outline,
        BorderSizePixel = 1,
...
function Linux.Create(config)
    ...
    local parentGui = RunService:IsStudio() and LocalPlayer.PlayerGui or game:GetService("CoreGui")
    ...
    function LinuxLib.Tab(config)
        ...
        local LeftContainer = Linux.Instance("ScrollingFrame", {
            Parent = TabContent,
            BackgroundColor3 = Color3.fromRGB(18, 18, 18),
            BorderColor3 = Linux.Theme.Outline,
            BorderSizePixel = 1,
            Size = UDim2.new(0.48, -5, 1, -30),
        ...
        local RightContainer = Linux.Instance("ScrollingFrame", {
            Parent = TabContent,
            BackgroundColor3 = Color3.fromRGB(18, 18, 18),
            BorderColor3 = Linux.Theme.Outline,
            BorderSizePixel = 1,
            ...
            AutomaticCanvasSize = Enum.AutomaticSize.Y,
            ScrollBarThickness = 4,
            ZIndex BawAlignment = Enum.HorizontalAlignment.Left,
            SortOrder = Enum.SortOrder.LayoutOrder
        })
        ...
        local TabElements = {}
        local elementOrder = {Left = 0, Right = 0}

        function TabElements.Button(config)
            elementOrder[config.Container or "Left"] = elementOrder[config.Container or "Left"] + 1
            local container = config.Container == "Right" and RightContainer or LeftContainer
            local BtnFrame = Linux.Instance("Frame", {
                ...
                ZIndex = 1,
                LayoutOrder = elementOrder[config.Container or "Left"]
            })
        ...
        function TabElements.Dropdown(config)
            ...
            local SelectedValue = IsMulti and (config.Default or {}) or (config.Default or (Options[1] or "None"))
            ...
            local function UpdateSelectedText()
                if IsMulti then
                    if #SelectedValue > 0 then
                        Selected.Text = table.concat(SelectedValue, ", ")
                    else
                        Selected.Text = "None"
                    end
                else
                ...
                if IsOpen then
                    for _, opt in pairs(Options) do
                        local OptBtn = Linux.Instance("TextButton", {
                            ...
                            Font = Enum.Font.Gotham,
                            Text = tostring(opt),
                            TextColor3 = (IsMulti and table.find(SelectedValue, opt)) or (opt == SelectedValue and Linux.Theme.SelectedOption) or Linux.Theme.Text,
                            TextSize = 14,
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            if IsMulti then
                                if table.find(SelectedValue, opt) then
                                    for i, v in ipairs(SelectedValue) do
                                        if v == opt then
                                            table.remove(SelectedValue, i)
                                            break
                                        end
                                    end
                                else
                                ...
                                OptBtn.TextColor3 = table.find(SelectedValue, opt) and Linux.Theme.SelectedOption or Linux.Theme.Text
                                UpdateSelectedText()
