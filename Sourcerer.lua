...
local Linux = {
    Theme = {
        ...
        ContainerOutline = Color3.fromRGB(50, 50, 50)
    }
}
...
function Linux.Create(config)
    ...
    function LinuxLib.Tab(config)
        ...
        local Container1 = Linux.Instance("ScrollingFrame", {
            ...
            ZIndex = 1,
            BorderSizePixel = 1,
            BorderColor3 = Linux.Theme.ContainerOutline
        })
        ...
        local function AnimateTabTransition()
            local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            TweenService:Create(Container1, tweenInfo, {Position = UDim2.new(1, 0, 0, 35)}):Play()
            TweenService:Create(Container2, tweenInfo, {Position = UDim2.new(1.52, 0, 0, 35)}):Play()
            task.wait(0.15)
            TweenService:Create(Container1, tweenInfo, {Position = UDim2.new(0, 5, 0, 35)}):Play()
            TweenService:Create(Container2, tweenInfo, {Position = UDim2.new(0.52, 0, 0, 35)}):Play()
        end
        ...
        function TabElements.Button(config)
            local container = config.Container == "Container2" and Container2 or Container1
            local elementOrder = config.Container == "Container2" and elementOrder2 or elementOrder1
            if config.Container == "Container2" then
                ...
                if IsOpen then
                    for _, opt in pairs(Options) do
                        local OptBtn = Linux.Instance("TextButton", {
                            ...
                            Font = Enum.Font.Gotham,
                            Text = tostring(opt),
                            TextColor3 = Linux.Theme.SelectedOption,
                            TextSize = 14,
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            ...
                            for _, btn in pairs(DropFrame:GetChildren()) do
                                if btn:IsA("TextButton") then
                                    btn.TextColor3 = Linux.Theme.SelectedOption
                                end
                            end
                ...
                if IsOpen then
                    for _, opt in pairs(Options) do
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            ...
                            OptBtn.TextColor3 = Linux.Theme.SelectedOption
                            UpdateSelectedText()
